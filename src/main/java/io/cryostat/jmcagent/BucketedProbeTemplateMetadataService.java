...
@ApplicationScoped
@LookupIfProperty(
        name = ConfigProperties.STORAGE_METADATA_PROBE_TEMPLATES_STORAGE_MODE,
        stringValue = ArchivedRecordingMetadataService.METADATA_STORAGE_MODE_BUCKET)
class BucketedProbeTemplateMetadataService
        implements CRUDService<String, ProbeTemplate, ProbeTemplate> {
    ...
    @ConfigProperty(name = ConfigProperties.AWS_METADATA_PREFIX_PROBE_TEMPLATES)
    String prefix;
    ...
    @Override
    public List<ProbeTemplate> list() throws IOException {
    ...
    @Override
    public void create(String k, ProbeTemplate template) throws IOException {
        storage.putObject(
    ...
    @Override
    public Optional<ProbeTemplate> read(String k) throws IOException {
        try (var stream =
                storage.getObject(
                        GetObjectRequest.builder().bucket(bucket).key(prefix(k)).build())) {
            var pt = new ProbeTemplate();
            pt.setFileName(k);
            pt.deserialize(stream);
