...
cleanup() {
    podman pod stop cryostat3-pod
    podman pod rm cryostat3-pod
}
trap cleanup EXIT
...
if [ -z "$KEYSTORE_PATH" ] && [ -f "$(dirname "$0")/certs/cryostat-keystore.p12" ] ; then
    ...
    FLAGS+=("-e QUARKUS_HTTP_SSL_CERTIFICATE_KEY_STORE_FILE=/certs/cryostat-keystore.p12")
    FLAGS+=("-e QUARKUS_HTTP_SSL_CERTIFICATE_KEY_STORE_PASSWORD=$(cat "$(dirname "$0")"/certs/keystore.pass)")
    FLAGS+=("-e QUARKUS_HTTP_INSECURE_REQUESTS=redirect")
fi
...
if ! podman pod exists cryostat3-pod; then
    podman pod create \
        --hostname cryostat \
        --name cryostat3-pod \
